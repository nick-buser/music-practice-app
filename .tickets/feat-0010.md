...
- [x] Fixture MIDI files (C major scale, A minor arpeggio, a chromatic run)
      yield the expected `key_guess`, `note_count`, and a histogram summing
      to 1 ± 1e-6 — `tests/test_midi_features.py` (substrate: unit)
- [x] Uploading a `.mid` asset creates one queued run keyed on the asset
      hash; uploading the same bytes again does not create a second run
      (substrate: unit)
- [x] `PropertiesPanel` renders a lineage badge per property — RTL test
      (substrate: unit)
- [x] After merge: upload a `.mid` on the dev slot; the idea page shows a
