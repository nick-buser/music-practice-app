...
**Acceptance criteria:**
- [x] `toMei()` is byte-identical across two runs and matches the committed
      MEI snapshot for every fixture — `cd app && npm run test` `mei.test.ts`
      (substrate: unit)
- [x] `renderScoreDoc` SVG is byte-identical across two calls on one shared
      toolkit and across two fresh toolkits for every fixture —
      `render.test.ts` (substrate: unit)
- [x] For every fixture, every ScoreDoc element id except `StaffDef.id`
...
- [x] `getMIDIValuesForElement(id).pitch === midiOf(note.pitch)` for every
...
- [x] `validateScoreDoc` rejects each negative fixture (overfull voice,
...
- [x] `seededIdSource` with one seed mints the same sequence twice; every
...
- [x] `canonicalJson` matches the RFC 8785 test vectors; `scoreDocHash`
      ignores `revision` — `canonical-json.test.ts` (substrate: unit)
- [x] The legacy ABC paths are behaviourally unchanged (only
      `resetOptions()` added): existing vitest suites pass unchanged; gates
      green (typecheck, vitest, build) (substrate: unit)
- [x] `sh docs/probes/verovio/run-all.sh` re-run: the `verovio 4.5.1-…`
